 ...
 */
class helper_plugin_pluginrepo_version extends Plugin
{
    public function execute()
    {

        if (!$this->getConf('github_user')) {
            return;
        }
        if (!$this->getConf('github_key')) {
            return;
        }
        ...
        foreach ($extensions as $extension) {
            ...
            if ($github) {
                [$out1, $out2] = $this->getDiscrepancies($extension, $github);
                $list1 .= $out1;
                $list2 .= $out2;
            }
        }
    ...
    protected function getDiscrepancies($plugindata, $githubdata)
    {
        ...
        $date2error = '';
        $nameerror  = '';
        if ($plugindata['lastupdate'] != $githubdata['date']) {
            $date1error = ' :!:';
        }
        if ($plugindata['lastupdate'] < $githubdata['gitpush']) {
            $date2error = ' :!:';
        }
        if ($basename != $githubdata['base']) {
            $nameerror = ' :!:';
        }

        if (!$date1error && !$date2error && !$nameerror) {
            return ['', ''];
        }
    ...
    {
        if (empty($plugindata['sourcerepo'])) {
            return false;
        }
        if (!preg_match('/github\.com\/([^\/]+)\/([^\/]+)/i', $plugindata['sourcerepo'], $m)) {
            return false;
        }
        $user = $m[1];
        $repo = $m[2];
